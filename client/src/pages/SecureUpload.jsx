...
const SecureUpload = () => {
  ...
  const uploadFile = async (file, type, timestamp, signature) => {
    ...
    const folder = type === 'images' ? 'images' : 'videos';
  ...
  const handleUpload = async (e) => {
    try {
      ...
      // Upload image file
      const imgUrl = await uploadFile(imageName, 'image', imgTimestamp, imgSignature);
      // Upload video file
      const videoUrl = await uploadFile(videoName, 'video', videoTimestamp, videoSignature);

      // Uncomment below if backend API is set up
      await axios.post(`${import.meta.env.VITE_BACKEND_BASEURL}/api/sign-upload`, { imgUrl, videoUrl });
